...
custom_model = CustomLinearRegression(learning_rate=0.1, n_iter=5000)
...
plt.plot(X_test, y_predictions_custom, color='blue', linestyle='dashed', label='Custom')
plt.xlabel('BMI')
...
print("\nПредсказания (первые 20 строк):")
print(" {:^20}  {:^30}  {:^30} ".format(
    "Реальные значения", "Предсказание Sklearn", "Предсказание Custom"))


for i in range(min(20, len(results))):
    print(" {:>20.2f} {:>30.2f}  {:>30.2f} ".format(
        results.iloc[i, 0], results.iloc[i, 1], results.iloc[i, 2]))
